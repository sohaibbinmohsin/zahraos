...
The three interfaces above (`DecideApplicationPayload`, `VerifyHoursPayload`, `BulkAssignHoursPayload`) were verified against the live `youth-republic/backend/supabase/functions/{decide-application,verify-hours,bulk-assign-hours}/handler.ts` source directly while writing this plan — not reconstructed from memory. Trust them as written; no re-verification step needed here.
...
- **Corrected during self-review**: an earlier draft of this plan reconstructed `decideApplication`/`verifyHours`/`bulkAssignHours`'s input shapes from an earlier session's memory rather than the live source, and got all three wrong — `organizationId` doesn't exist on the first two (both derive it server-side from the target row), `verifyHours` has a required `decision: "verified" | "rejected"` field that was missing entirely, and `bulkAssignHours` takes a specific `participationIds: string[]` rather than a blanket opportunity+date assignment. Caught by reading the real `handler.ts` files directly before finalizing, not by testing — fixed throughout Task 1 (`lib/vmsFunctions.ts`), Task 6 (Applications screen), and Task 7 (Hours screen + `BulkAssignHoursForm`, which now takes a `participants` list and renders checkboxes rather than assigning to everyone).
